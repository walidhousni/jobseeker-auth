...
import { IPaginateProps, ISearchResult } from "@walidhousni/jobseeker-shared";
...
export async function gigs(req: Request, res: Response): Promise<void> {
    ...
    res.status(StatusCodes.OK).json({ message: 'Search gigs resulsts', total: gigs.total, gigs: resultHits });
}
